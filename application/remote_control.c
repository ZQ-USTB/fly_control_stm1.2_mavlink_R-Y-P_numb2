...
//  SET_BIT(huart3.Instance->CR1, USART_CR1_RE);
	if(HAL_UARTEx_ReceiveToIdle_DMA(&huart3, elrs_data_temp, MAX_FRAME_SIZE)!=HAL_OK)
	{
		 Error_Handler();
	}
...
{
    ELRS_Init();
//	  HAL_UARTEx_ReceiveToIdle_DMA(&huart3, elrs_data_temp, MAX_FRAME_SIZE); // ÿжϽ
//    __HAL_DMA_DISABLE_IT(&hdma_usart3_rx, DMA_IT_HT);                      // رDMAж
    ...
    if (elrs_data_temp[0] == CRSF_ADDRESS_FLIGHT_CONTROLLER)
    {
        ...
        else if (elrs_data_temp[2] == CRSF_FRAMETYPE_RC_CHANNELS_PACKED) // ֡ΪRCͨ
        {
